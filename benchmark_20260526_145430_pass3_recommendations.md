# Pass-3 root-cause analysis & remediation suggestions

_45 remaining BAD assessments after pass-2 (82.6% pass rate)._

## Summary

| Root cause | Count | Recommended action |
|---|---:|---|
| **NONDETERMINISM_RANK_VS_LIMIT** | 36 | Don't chase. Genie alternates between `RANK() OVER` window-CTE and plain `ORDER BY + LIMIT` shapes on top-N questions; both are correct. Soft cap = 80-85% on this corpus until Databricks' judge weighs SQL semantics over shape. |
| **WRONG_TABLE_OR_METRIC** | 6 | Add a targeted `genie_instructions` line per affected spec: "For X questions, prefer table Y_metrics." Or add the missing measure as a SQL snippet. |
| **GENUINE_HUMAN_REVIEW** | 2 | Read the SQL pair manually. |
| **EXTRA_ANNOTATION_COLUMN** | 1 | Add to `genie_instructions`: "Never add source-table label columns to result sets." |

## Why pass-3 doesn't ship code

Pass-2 broadened the PROMOTE rule to swap gold with Genie's last-run output. The new benchmark run got *different* Genie output for ~25 of these questions — Genie alternates between `RANK() OVER` and `LIMIT N` shapes for top-N requests, and the judge measures shape-equality, not semantic equality. Re-promoting would just reverse pass-2 without lifting the rate.

The high-leverage remaining work is **per-spec semantic** — `genie_instructions` lines steering the model to a canonical table or measure, or widening metric views to expose missing dims. That's hand-curated, not mechanical, so it belongs in a follow-up curated commit rather than another `--fix-bench-*` mode.

## NONDETERMINISM_RANK_VS_LIMIT (36)

_Gold has RANK()/window-CTE machinery; new actual uses plain ORDER BY + LIMIT (or vice-versa). Genie alternates between equivalent shapes._

**Recommendation:** Don't chase. Genie alternates between `RANK() OVER` window-CTE and plain `ORDER BY + LIMIT` shapes on top-N questions; both are correct. Soft cap = 80-85% on this corpus until Databricks' judge weighs SQL semantics over shape.

### logistics/load_demand_forecasting

**Q:** Which lane categorys have the highest number of forecast records?

**Genie's complaint:** RESULT_EXTRA_ROWS, LLM_JUDGE_INCOMPLETE_OR_PARTIAL_OUTPUT
...
**Suggested fix:** Accept the variance — no code action. Re-running the benchmark will sometimes pass and sometimes fail on the same question.
...
**Genie's complaint:** RESULT_MISSING_ROWS, LLM_JUDGE_INCORRECT_METRIC_CALCULATION
...
**Genie's complaint:** RESULT_MISSING_ROWS, LLM_JUDGE_MISSING_OR_INCORRECT_FILTER
...
**Genie's complaint:** RESULT_EXTRA_ROWS, LLM_JUDGE_INCORRECT_TABLE_OR_FIELD_USAGE
...
**Genie's complaint:** RESULT_MISSING_COLUMNS, LLM_JUDGE_INCOMPLETE_OR_PARTIAL_OUTPUT
...
**Genie's complaint:** EMPTY_RESULT
...
**Genie's complaint:** RESULT_MISSING_COLUMNS, LLM_JUDGE_INCORRECT_TABLE_OR_FIELD_USAGE
...
**Genie's complaint:** RESULT_MISSING_ROWS, LLM_JUDGE_INCOMPLETE_OR_PARTIAL_OUTPUT
...
**Genie's complaint:** RESULT_MISSING_COLUMNS, LLM_JUDGE_MISSING_OR_INCORRECT_FILTER
...
**Genie's complaint:** RESULT_EXTRA_ROWS, LLM_JUDGE_MISSING_OR_INCORRECT_FILTER
...
**Genie's complaint:** RESULT_MISSING_COLUMNS, LLM_JUDGE_MISSING_OR_INCORRECT_AGGREGATION
...
**Genie's complaint:** RESULT_EXTRA_ROWS, LLM_JUDGE_MISSING_OR_INCORRECT_AGGREGATION
...
**Genie's complaint:** RESULT_EXTRA_ROWS, LLM_JUDGE_FORMATTING_ERROR
...
## WRONG_TABLE_OR_METRIC (6)

_Genie picked a different table or computed metric than gold expects. The question is ambiguous about which canonical source to use._

**Recommendation:** Add a targeted `genie_instructions` line per affected spec: "For X questions, prefer table Y_metrics." Or add the missing measure as a SQL snippet.

### machinery/production_monitoring

**Q:** What is the monthly trend in total output units?

**Genie's complaint:** RESULT_MISSING_ROWS, LLM_JUDGE_INCORRECT_TABLE_OR_FIELD_USAGE
...
**Suggested fix:** Genie chose `production_runs_metrics`, gold expects `oee_metrics`. Add to `genie_instructions`: "For questions about 'what is the monthly trend in total output units?…', use `oee_metrics`."
...
**Suggested fix:** Genie chose `fleet_operations_metrics`, gold expects `fleet_operations`. Add to `genie_instructions`: "For questions about 'which vehicle class: heavy duty, medium duty, light duty, sp…', use `fleet_operations`."
...
**Suggested fix:** Genie chose `outcome_snapshots`, gold expects `simulation_runs`. Add to `genie_instructions`: "For questions about 'how many distinct transactions appeared each month?…', use `simulation_runs`."
...
**Suggested fix:** Genie chose `maintenance_events_metrics`, gold expects `maintenance_events`. Add to `genie_instructions`: "For questions about 'how has highest downtime hours changed over time?…', use `maintenance_events`."
...
**Suggested fix:** Genie chose `sensor_readings_metrics`, gold expects `equipment_health_monthly`. Add to `genie_instructions`: "For questions about 'which process units have the highest average asset health sc…', use `equipment_health_monthly`."
...
**Suggested fix:** Genie chose `production_readings`, gold expects `production_readings_metrics`. Add to `genie_instructions`: "For questions about 'which wells have the highest total daily gas production in m…', use `production_readings_metrics`."

---

## GENUINE_HUMAN_REVIEW (2)

_Doesn't fit a pattern — needs eyeballing._

**Recommendation:** Read the SQL pair manually.
...
## EXTRA_ANNOTATION_COLUMN (1)

_Genie added a debug / label column (e.g. literal 'condition_readings' AS source) that gold doesn't have._

**Recommendation:** Add to `genie_instructions`: "Never add source-table label columns to result sets."

### machinery/asset_health

**Q:** How many distinct transactions appeared each month?

**Genie's complaint:** RESULT_EXTRA_ROWS, LLM_JUDGE_MISINTERPRETATION_OF_USER_REQUEST
...
**Suggested fix:** Genie added literal column(s) ['source', 'source', 'source']. Add a `genie_instructions` rule banning them.
